...
{
    internal class PetPredictiveRingAttack : Behavior
    {
        private static readonly Dictionary<Tuple<int, float, float, int>, PetPredictiveRingAttack> instances =
            new Dictionary<Tuple<int, float, float, int>, PetPredictiveRingAttack>();
        ...
        private PetPredictiveRingAttack(int count, float predictFactor, float radius, int projectileIndex)
        {
            this.count = count;
        ...
        protected override bool TickCore(RealmTime time)
        {
            ...
            float dist = radius;
            Entity entity = radius == 0 ? null : GetNearestEntityPet(ref dist);

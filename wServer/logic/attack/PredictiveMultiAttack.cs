...
{
    internal class PredictiveMultiAttack : Behavior
    {
        private static readonly Dictionary<Tuple<float, float, int, float, int>, PredictiveMultiAttack> instances =
            new Dictionary<Tuple<float, float, int, float, int>, PredictiveMultiAttack>();
        ...
        public static PredictiveMultiAttack Instance(float radius, float angle, int numShot, float predictFactor,
        ...
        protected override bool TickCore(RealmTime time)
        {
            ...
            if (entity != null)
            {
                ...
                double angleOffset = Predict(entity, desc);
                double startAngle = Math.Atan2(entity.Y - chr.Y, entity.X - chr.X) + angleOffset - angle*(numShot - 1)/2;

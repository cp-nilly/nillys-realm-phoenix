...
public class FameStats
{
    ...
    public int CalculateTotal(Account acc, Char chr, int baseFame, out bool firstBorn)
    {
        ...
        for (int i = 0; i < 4; i++) //Well Equipped
        {
            ...
            int b = XmlDatas.ItemDescs[chr.Equipment[i]].FameBonus;
            if (b > 0)
        ...
        return baseFame + bonus;
    }

    private void SerializeBonus(XmlDocument doc, Account acc, Char chr, int baseFame, bool firstBorn)
    {
        if (chr.CharacterId < 2) //Ancestor
        {
